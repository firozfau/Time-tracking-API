...
class GeneralModel:
    ...
    async def eventNameList(self, data):
        ...
        sqlQuery = text(
            """
            SELECT *  from event_name_list where status=1
            """
        )
    ...
    async def accessIPList(self, data):
        ...
        sqlQuery = text(
            """
           SELECT tts_access_ip_list.office_id,tts_access_ip_list.ip_address ,tts_access_ip_list.comments,tts_access_ip_list.status,
            tts_office_config.*
            from tts_access_ip_list
            INNER JOIN tts_office_config on tts_access_ip_list.office_id=tts_office_config.id
            where tts_access_ip_list.status=1
            """
        )
    ...
    async def officeList(self, data):
        ...
        sqlQuery = text(
            """
           SELECT * from tts_office_config where `status`=1
            """
        )
    ...
    async def addNewOfficeInformation(self, data,objectData):
        ...
        sqlQuery = text(
            """
            INSERT INTO tts_office_config (`country_id`, `organization_name`, `address`, `main_responsible`, `director_name`, `finance_name`, `office_email`, `office_mobile_number`, `default_langauge`, `office_start_date`, `default_timezone`, `default_work_start_time`, `default_work_end_time`, `default_weekend_id`, `default_office_ip`, `comments`, `created_id`, `user_ip` )
            VALUES (:country_id,:organization_name,:address,:main_responsible,:director_name,:finance_name,:office_email,:office_mobile_number,:default_langauge,:office_start_date,:default_timezone,:default_work_start_time,:default_work_end_time,:default_weekend_id,:default_office_ip,:comments,:created_id,:user_ip)
            """
        )
        #print(sqlQuery)
    ...
    async def updateUserConfig(self, data):
        try:
            ...
            sql_query = text(
                "UPDATE tts_user_config SET `office_id` =:office_id,`user_id` = :user_id,`country_id` = :country_id,`time_zone` = :time_zone, `work_start_time` =:work_start_time,`work_end_time` = :work_end_time,`weekend_id` = :weekend_id,`comments` = :comments,`updated_id` = :updated_id,`user_ip` = :user_ip  WHERE id = :user_config_id"
            )
