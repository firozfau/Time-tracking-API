...
@general.post("/api/countriesList")
async def country_list_general(request: CheckIPSkeleton):
    is_valid, errors = Validation. checkIPRequest(request)
    if not is_valid:
        return {"status": "error", "message": "Validation", "data":errors}    # Return errors along with success status
    else:
        data = await GeneralController.countriesList(request)
        return data
...
@general.post("/api/usersList")
async def users_list_general(request: CheckIPSkeleton):
    is_valid, errors = Validation. checkIPRequest(request)
    if not is_valid:
        return {"status": "error", "message": "Validation", "data":errors}    # Return errors along with success status
    else:
        data = await GeneralController.userAllList(request)
        return data
...
async def add_new_office_general(request: OfficeConfigSkeleton):
    ...
    if not is_valid:
       ...
       # error_obj= JsonFormate.getEncodeData(errors)
        return {"status": "error", "message": "Validation", "data":errors}    # Return errors along with success status
    else:
...
@general.post("/api/userConfigAllList")
async def user_config_all_list_general(request: CheckIPSkeleton):
    is_valid, errors = Validation. checkIPRequest(request)
    if not is_valid:
        return {"status": "error", "message": "Validation", "data":errors}    # Return errors along with success status
    else:
        data = await GeneralController.userConfigAllList(request)
        return data
